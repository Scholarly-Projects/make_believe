...
The interface is designed to provide the reader with only the most relevant reference material. Citations appear to the right of the passage they are referencing as the reader scrolls through and image citations appear below figures as they are arranged by the author in markdown files for each chapter. This data is informed by CSV files which automatically generate scholarly formatted bibliography and image credit pages, so adjusting one file will update this reference material across all parts of the site. Additionally, if you need to add or remove citations through the editing process,, a simple Python script is included to update markdown citation numbers across all of the chapters seamlessly.
